...
import { bulkDismissTargets } from "./bulkDismiss";
...
export function ReviewQueuePage({ navigate, onChange }: { navigate: Navigate; onChange: () => void }) {
  ...
  async function dismissAll(kind: string, label: string) {
    const result = await backend.reviews.dismissAll(kind);
    setNotice(`Dismissed ${result.dismissed} "${label}" item${result.dismissed === 1 ? "" : "s"}.`);
    await load();
    onChange();
  }
  ...
  return <>
    ...
    <div className="review-page">
      ...
      {bulkDismissTargets(items).length > 0 && <div className="decision-actions">{bulkDismissTargets(items).map(target => <button key={target.kind} className="button" onClick={() => dismissAll(target.kind, target.label).catch(error => setNotice(error.message))}>Dismiss all {target.count} “{target.label}” items</button>)}</div>}
