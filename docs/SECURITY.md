...
- LLM redirects are disabled, host-checked, and TLS verification is mandatory for chat and model-catalog requests. Keys pasted in Settings are encrypted with Windows DPAPI for the current Windows account and stored under local application data, outside OneDrive; the per-user DPAPI binding is the credential control, not the file location. The locally saved key is primary, and `GENAI_API_KEY` is a fallback only when no local key exists. The API never returns a saved key, although Python necessarily holds the decrypted key in process memory while preparing and sending a request.
