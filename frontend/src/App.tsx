...
function ProjectArchiveWorkspace({ projectId, navigate }: { projectId: string; navigate: (path: string) => void }) {
  ...
  async function removeSource(source: Source) {
    ...
    const reason = answer.trim() || "Removed from active project memory by the user.";
    await api.post(`/api/projects/${projectId}/sources/${source.id}/remove`, { reason });
  ...
  async function restoreSource(source: Source) {
    const restored = await api.post<Source>(`/api/projects/${projectId}/sources/${source.id}/restore`);
    setSourceDetail(null);
  ...
  async function rebuildKnowledge() {
    const result = await api.post<{ active_sources: number; knowledge_items: number }>(`/api/projects/${projectId}/knowledge/rebuild`);
  ...
  const visibleSources = project.sources.filter((source, index, all) => {
  ...
  return <>
    ...
    <div className="archive-workspace">
      ...
      {view === "summary" && <section className="panel summary-panel">
        ...
        {project.living_summary.generation_state !== "current" && <div className={`notice ${project.living_summary.generation_state === "failed" ? "error" : ""}`}>Summary is {project.living_summary.generation_state}. {project.living_summary.error || "Regenerate it to build a summary from the current active knowledge."}</div>}
      ...
      {view === "sources" && <section className="panel"><header><div><small>Self-contained OneDrive packages; removed items remain recoverable</small><h2>Sources</h2></div><div className="header-actions"><button className="button" onClick={() => rebuildKnowledge().catch(error => setNotice(error.message))}>Update project knowledge</button><button className="button" onClick={() => api.post("/api/sources/retry-pending").then(() => pollRefresh(load)).catch(error => setNotice(error.message))}>Retry pending</button></div></header><div className="source-list">{visibleSources.map(source => <article key={source.id}><div><small>{source.ingestion_id || "Linked source"} / {new Date(source.created_at).toLocaleString()}</small><strong>{source.source_title || source.original_filename}</strong><p>{source.source_summary || "Derived summary pending."}</p>{source.memory_state === "removed" && <p>This package is archived and excluded from active project memory.</p>}{source.error_message && <p>{source.error_message}</p>}</div><span className={`state ${source.memory_state}`}>{source.memory_state}</span><span className={`state ${source.processing_state}`}>{source.processing_state.replace("_", " ")}</span><button className="button compact" onClick={() => api.get<Source>(`/api/sources/${source.id}`).then(setSourceDetail).catch(error => setNotice(error.message))}>Inspect package</button><a className="button compact" href={`/api/sources/${source.id}/original`}>Open original</a>{source.ingestion_path && (source.memory_state === "removed" ? <button className="button primary compact" onClick={() => restoreSource(source).catch(error => setNotice(error.message))}>Restore to project</button> : <button className="button compact" onClick={() => removeSource(source).catch(error => setNotice(error.message))}>Remove from project</button>)}</article>)}</div></section>}
    ...
    {sourceDetail && <Modal title={sourceDetail.source_title || sourceDetail.original_filename} onClose={() => setSourceDetail(null)}><div className="source-package-detail"><p>{sourceDetail.source_summary}</p><small>{sourceDetail.ingestion_id} / {sourceDetail.capture_method} / memory: {sourceDetail.memory_state}</small><div className="header-actions"><button className="button" onClick={() => refreshDerived(sourceDetail.id).catch(error => setNotice(error.message))}>Rebuild derived files</button>{sourceDetail.ingestion_path && (sourceDetail.memory_state === "removed" ? <button className="button primary" onClick={() => restoreSource(sourceDetail).catch(error => setNotice(error.message))}>Restore to project</button> : <button className="button" onClick={() => removeSource(sourceDetail).catch(error => setNotice(error.message))}>Remove from project</button>)}</div><h3>Original files</h3>{sourceDetail.original_files?.map(file => <article key={file.id}><div><strong>{file.original_name}</strong><small>{file.relative_path} / SHA-256 {file.sha256}</small></div><a className="button compact" href={`/api/original-files/${file.id}`}>Open original</a></article>)}{sourceDetail.lifecycle?.length ? <details><summary>Source history ({sourceDetail.lifecycle.length})</summary>{sourceDetail.lifecycle.map(event => <article key={event.id}><strong>{event.event_type.replaceAll("_", " ")}</strong><small>{new Date(event.created_at).toLocaleString()} / {event.reason}</small></article>)}</details> : null}<details><summary>Manifest</summary><pre>{JSON.stringify(sourceDetail.manifest, null, 2)}</pre></details></div></Modal>}
...
function ReviewQueue({ navigate, onChange }: { navigate: (path: string) => void; onChange: () => void }) {
  ...
  useEffect(() => {
    ...
    const recommendation = proposed?.recommended_project_id as string | undefined;
  ...
  async function resolve(action: "apply" | "dismiss" | "keep" | "move" | "remove") {
    ...
    // cross_project_evidence remains supported for review rows created before source lifecycle migration 003.
    ...
    await api.post(`/api/reviews/${selected.id}/resolve`, { action, target_project_id: target || null, reason: reason?.trim() || null, rule: action === "apply" && selected.kind === "multi_project_route" ? rule : null, action_item_id: evidence?.action_item_id || null, field: fieldRecommendation?.field || null, value: fieldRecommendation?.value || null, ...(selected.kind === "inferred_action" ? actionDraft : {}) });
    setNotice(selected.kind === "project_fit" ? (action === "remove" ? "Source archived without changing project memory." : "Project choice confirmed; source processing resumed.") : action === "apply" ? (selected.kind === "multi_project_route" ? "Decision applied and routing rule saved." : "Decision applied.") : "Review item dismissed and retained for audit.");
    await load(); onChange();
  }
  return <>
